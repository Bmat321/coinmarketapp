const main = async () => {
  ...
  const actioCoinFactory = await hre.ethers.getContractFactory("Actioncoin");
  const actioCoinContract = await actioCoinFactory.deploy();
  await actioCoinContract.deployed();
  console.log("ActioncoinToken deployed to: ", actioCoinContract.address);
};
